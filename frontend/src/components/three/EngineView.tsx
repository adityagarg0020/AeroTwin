...
import { SmokeEffect, FireEffect, SparkEffect } from './SmokeFireEffects';
import { EnhancedAirflow } from './AirflowEnhanced';
import { useEngineStore } from '../../store/engineStore';

export function EngineView({ xray, heatmap }: { xray?: boolean; heatmap?: boolean }) {
  const { isExploded, predictions } = useEngineStore();
      ...
      <Canvas shadows gl={{ antialias: true, alpha: true, powerPreference: 'high-performance' }}>
        ...
        <Suspense fallback={null}>
          ...
          <group position={[0, -0.5, 0]}>
            <TurbojetEngine exploded={isExploded} xray={xray || false} />
            {heatmap && <HeatMapOverlay />}
            <EnhancedAirflow />
            {health < 0.5 && <><SmokeEffect /><FireEffect /><SparkEffect /></>}
          </group>
